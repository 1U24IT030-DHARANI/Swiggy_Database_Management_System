...
select *
from reviews
where DeliveryRating = 5 and FoodRating = 5;

-- 36. Display reviews with ratings less than three.
select *
from reviews
where DeliveryRating or FoodRating < 3;
...
SELECT ItemName,
       Price,
       ROUND(Price * 1.05) AS Price_With_ServiceCharge
FROM MenuItems;
...
SELECT 
    AVG(FoodRating), AVG(DeliveryRating)
FROM
    reviews;
    
-- 50. Display the average review rating rounded to two decimal places.
SELECT 
    ROUND(AVG(FoodRating), 2) AS AverageFoodRating,
    ROUND(AVG(DeliveryRating), 2) AS AverageDeliveryRating
FROM reviews;
...
select paymentmethod, count(*) as num_transactions
from payments
group by paymentmethod;

-- 55. Display the number of reviews for each rating.
select foodrating, count(*) as num_reviews
from reviews
group by foodrating;

-- 56. Display the number of menu items in each food category.
select mc.categoryname, count(mi.itemid) as num_items
from menucategories mc
left join menuitems mi on mc.categoryid = mi.categoryid
...
select mc.categoryname, avg(mi.price) as avg_price
from menucategories mc
join menuitems mi on mc.categoryid = mi.categoryid
group by mc.categoryname;
...
select concat(c.firstname,' ',c.lastname) as customername, o.orderid, o.orderdate, o.totalamount
from customers c
join orders o on c.customerid = o.customerid;

-- 62. Display order ID, restaurant name, order date, and order amount.
select o.orderid, r.restaurantname, o.orderdate, o.totalamount
from orders o
join restaurants r on o.restaurantid = r.restaurantid;

-- 63. Display menu item name, category name, and price.
select mi.itemname, mc.categoryname, mi.price
from menuitems mi
...
from delivery d
join deliverypartners dp on d.partnerid = dp.partnerid;

-- 66. Display customer name, review rating, and review comment.
select concat(c.firstname,' ',c.lastname) as customername, r.foodrating, r.deliveryrating, r.reviewcomment
from reviews r
join customers c on r.customerid = c.customerid;

-- 67. Display restaurant name, review rating, and review comment.
select rest.restaurantname, r.foodrating, r.deliveryrating, r.reviewcomment
from reviews r
join restaurants rest on r.restaurantid = rest.restaurantid;

-- 68. Display restaurant name, menu item, and menu price.
select r.restaurantname, mi.itemname, mi.price
from menuitems mi
...
select c.customerid, concat(c.firstname,' ',c.lastname) as customername, o.orderid, o.orderdate, o.totalamount
from customers c
...
select o.orderid, o.orderdate, d.deliverystatus, d.deliverytime
from orders o
left join delivery d on o.orderid = d.orderid;

-- 73. Display all restaurants along with their customer reviews.
select r.restaurantname, rv.foodrating, rv.deliveryrating, rv.reviewcomment
from restaurants r
...
select p.paymentid, p.amount, p.paymentmethod, p.paymentstatus, o.orderid, o.orderdate
from payments p
join orders o on p.orderid = o.orderid;

-- 76. Display all reviews with restaurant details.
select rv.reviewid, rv.foodrating, rv.deliveryrating, rv.reviewcomment, r.restaurantname, r.city
from reviews rv
...
select concat(c.firstname,' ',c.lastname) as customername, r.restaurantname, o.totalamount, p.paymentstatus
from orders o
...
select concat(c.firstname,' ',c.lastname) as customername, r.restaurantname, dp.partnername, d.deliverystatus
from orders o
join customers c on o.customerid = c.customerid
join restaurants r on o.restaurantid = r.restaurantid
left join delivery d on o.orderid = d.orderid
left join deliverypartners dp on d.partnerid = dp.partnerid;

-- 80. Display customer name, restaurant name, payment amount, payment method, and review rating.
select concat(c.firstname,' ',c.lastname) as customername, r.restaurantname, p.amount, p.paymentmethod, rv.foodrating
from orders o
join customers c on o.customerid = c.customerid
join restaurants r on o.restaurantid = r.restaurantid
left join payments p on o.orderid = p.orderid
...
select c.customerid, concat(c.firstname,' ',c.lastname) as customername, count(o.orderid) as total_orders
from customers c
left join orders o on c.customerid = o.customerid
...
select r.restaurantname, avg(rv.foodrating) as avg_food_rating, avg(rv.deliveryrating) as avg_delivery_rating
from restaurants r
...
select dp.partnername, count(d.deliveryid) as total_deliveries
from deliverypartners dp
left join delivery d on dp.partnerid = d.partnerid
...
select concat(c.firstname,' ',c.lastname) as customername, r.restaurantname
from reviews rv
join customers c on rv.customerid = c.customerid
join restaurants r on rv.restaurantid = r.restaurantid;

-- 88. Display restaurant name, city, and average menu price.
select r.restaurantname, r.city, avg(mi.price) as avg_menu_price
from restaurants r
...
select o.orderid, concat(c.firstname,' ',c.lastname) as customername, r.restaurantname, o.orderdate, o.totalamount,
    p.paymentmethod, p.paymentstatus, dp.partnername, d.deliverystatus, d.deliverytime
from orders o
join customers c on o.customerid = c.customerid
join restaurants r on o.restaurantid = r.restaurantid
left join payments p on o.orderid = p.orderid
...
select orderid, month(orderdate) as order_month
from orders;

-- 99. Display the month name for every order.
select orderid, monthname(orderdate) as month_name
from orders;

-- 100. Display the day of the month for every order.
select orderid, day(orderdate) as day_of_month
from orders;

-- 101. Display the weekday name for every order.
select orderid, dayname(orderdate) as weekday_name
from orders;

-- 102. Display the weekday number for every order.
select orderid, weekday(orderdate) as weekday_number
from orders;

-- 103. Display the week number for every order.
select orderid, week(orderdate) as week_number
from orders;

-- 104. Display the quarter for every order.
select orderid, quarter(orderdate) as order_quarter
from orders;

-- 105. Display the day number within the year for every order.
select orderid, dayofyear(orderdate) as day_of_year
from orders;

-- 106. Calculate the number of days between the order date and delivery date.
select o.orderid, datediff(d.deliverytime, o.orderdate) as days_between
from orders o
join delivery d on o.orderid = d.orderid;

-- 107. Calculate the delivery duration in minutes.
select o.orderid, timestampdiff(minute, d.pickuptime, d.deliverytime) as delivery_duration_minutes
from orders o
join delivery d on o.orderid = d.orderid;

-- 108. Display the expected delivery date by adding two days to the order date.
select orderid, orderdate, date_add(orderdate, interval 2 day) as expected_delivery_date
from orders;

-- 109. Display a reminder date three days before the order date.
select orderid, orderdate, date_sub(orderdate, interval 3 day) as reminder_date
from orders;

-- 110. Add seven days to each order date.
select orderid, orderdate, date_add(orderdate, interval 7 day) as order_date_plus_7
from orders;

-- 111. Subtract five days from each order date.
select orderid, orderdate, date_sub(orderdate, interval 5 day) as order_date_minus_5
from orders;
...
select orderid, date_format(orderdate, '%d-%m-%Y') as order_date_formatted
from orders;

-- 114. Display the order month and year in "Month YYYY" format.
select orderid, date_format(orderdate, '%M %Y') as month_year
from orders;

-- 115. Display monthly revenue generated from completed payments.
select date_format(p.paymentdate, '%Y-%m') as month, sum(p.amount) as monthly_revenue
from payments p
...
select date_format(orderdate, '%Y-%m') as order_month, count(*) as total_orders
from orders
group by date_format(orderdate, '%Y-%m')
order by order_month;

-- 118. Display the total number of orders placed on each weekday.
select dayname(orderdate) as weekday_name, count(*) as total_orders
from orders
group by dayname(orderdate);

-- 119. Display the average delivery time in minutes.
select avg(timestampdiff(minute, pickuptime, deliverytime)) as avg_delivery_minutes
from delivery
where pickuptime is not null and deliverytime is not null;
...
select customerid, customername, total_spending,
    row_number() over(order by total_spending desc) as row_num
from (
    select c.customerid, concat(c.firstname,' ',c.lastname) as customername, sum(o.totalamount) as total_spending
    from customers c
    join orders o on c.customerid = o.customerid
    group by c.customerid, customername
) as spending;
...
select restaurantname, total_revenue,
...
select paymentid, orderid, amount,
    lag(amount) over(order by paymentdate) as previous_payment_amount
from payments;

-- 126. Display each payment along with the next payment amount.
select paymentid, orderid, amount,
    lead(amount) over(order by paymentdate) as next_payment_amount
from payments;
...
select paymentid, paymentdate, amount,
    avg(amount) over(order by paymentdate rows between 2 preceding and current row) as moving_avg
from payments;
...
select o.orderid, o.restaurantid, o.totalamount
from orders o
where o.totalamount > (
...
select orderid, totalamount,
    case
        ...
        when totalamount between 300 and 700 then 'Medium'
        else 'High'
...
select
    sum(case when paymentstatus = 'Success' then 1 else 0 end) as completed_count,
    ...
    sum(case when paymentstatus = 'Refunded' then 1 else 0 end) as refunded_count
from payments;
...
select concat(c.firstname,' ',c.lastname) as customername, sum(o.totalamount) as total_spending
from customers c
join orders o on c.customerid = o.customerid
...
select r.restaurantname, avg(rv.foodrating) as avg_rating
from restaurants r
join reviews rv on r.restaurantid = rv.restaurantid
group by r.restaurantname
...
select c.customerid, concat(c.firstname,' ',c.lastname) as customername,
    coalesce(sum(o.totalamount), 0) as customer_lifetime_value
from customers c
left join orders o on c.customerid = o.customerid
group by c.customerid, customername;
...
select hour(orderdate) as order_hour, count(*) as total_orders
from orders
group by hour(orderdate)
order by order_hour;

-- 143. Display the average delivery time for each delivery partner.
select dp.partnername, avg(timestampdiff(minute, d.pickuptime, d.deliverytime)) as avg_delivery_minutes
from deliverypartners dp
join delivery d on dp.partnerid = d.partnerid
where d.pickuptime is not null and d.deliverytime is not null
group by dp.partnername;
...
select paymentmethod, count(*) as completed_transactions,
    rank() over(order by count(*) desc) as method_rank
from payments
where paymentstatus = 'Success'
group by paymentmethod;

-- 146. Display each restaurant's revenue along with its percentage contribution to total revenue.
select r.restaurantname, sum(o.totalamount) as restaurant_revenue,
    round(sum(o.totalamount) * 100.0 / (select sum(totalamount) from orders), 2) as revenue_percentage
from restaurants r
join orders o on r.restaurantid = o.restaurantid
group by r.restaurantname;
...
select c.customerid, concat(c.firstname,' ',c.lastname) as customername, count(distinct o.restaurantid) as num_restaurants
from customers c
join orders o on c.customerid = o.customerid
group by c.customerid, customername
...
select
    ...
    (select count(*) from payments where paymentstatus = 'Success') as completed_payments,
...
select r.restaurantname,
    ...
    avg(rv.foodrating) as avg_customer_rating,
    avg(timestampdiff(minute, d.pickuptime, d.deliverytime)) as avg_delivery_time_minutes
from restaurants r
left join orders o on r.restaurantid = o.restaurantid
left join reviews rv on r.restaurantid = rv.restaurantid
left join delivery d on o.orderid = d.orderid
